...
def parse_input_arguments():
    ...
    parser.add_argument(
        "--hidden_units", type=int, default=DEFAULT_HIDDEN_UNITS, help="Number of hidden units"
    )
...
def get_number_of_classes(train_directory, valid_directory, test_directory):
    ...
    if (
        (number_train_classes != number_valid_classes)
        or (number_train_classes != number_test_classes)
        or (number_valid_classes != number_test_classes)
    ):
...
def load_datasets(train_directory, valid_directory, test_directory):
    ...
    train_transforms = torchvision.transforms.Compose(
        [
            torchvision.transforms.RandomRotation(DEGREES_ROTATION),
            torchvision.transforms.RandomResizedCrop(SIZE_CROP),
    ...
    train_data = torchvision.datasets.ImageFolder(train_directory, transform=train_transforms)
    valid_data = torchvision.datasets.ImageFolder(valid_directory, transform=valid_transforms)
    test_data = torchvision.datasets.ImageFolder(test_directory, transform=test_transforms)
    ...
    classifier = torch.nn.Sequential(
        OrderedDict(
            [
                ...
                ("drop", torch.nn.Dropout(p=DROPOUT_PROBABILITY)),
                ("relu", torch.nn.ReLU()),
                ("fc2", torch.nn.Linear(out_features, number_classes)),
